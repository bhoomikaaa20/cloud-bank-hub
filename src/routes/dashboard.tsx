...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [{ data: a }, { data: t }, { data: p }] = await Promise.all([
        ...
        supabase.from("transactions").select("*").or(`sender_id.eq.${user.id},receiver_id.eq.${user.id}`).order("created_at", { ascending: false }).limit(5),
  ...
  const createAccount = async () => {
    ...
    if (!name) return;
    const { error } = await supabase.from("accounts").insert({ user_id: user!.id, account_name: name });
    if (error) toast.error(error.message); else toast.success("Account created with $0 balance");
  };
